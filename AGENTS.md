...
- Self-update: `plugin-update-checker` (vendor/plugin-update-checker/),
  pulls releases from GitHub
...
## Version bumps
Every release needs the version updated in THREE places, or the updater
won't detect a new version / the shipped zip filename will mismatch:
1. `Version:` in the plugin header comment (wp-table-builder.php)
2. `WTB_VERSION` constant (wp-table-builder.php)
3. hardcoded `version` variable in `package.py` (used for the zip filename)
